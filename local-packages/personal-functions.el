;;; personal-functions.el --- Personal functions -*- lexical-binding: t -*-
...
(defun cc/backward-kill-path-component ()
  "Kills backward path component until root.
NOTE: doesn't take into account escapes.
NOTE: works only on POSIX paths."
  (let ((end-point (point)))
    (when (eq ?/ (char-after (1- (point))))
      (backward-char))
    (search-backward "/" (line-beginning-position) t)
    (when (eq (point) (1- end-point))
      (beginning-of-line))
...
(defun cc/duplicate-line-or-region-above (n)
  "Duplicate N times the current line or region above the original."
  (interactive "p")
  (cc/duplicate-line-or-region (- n)))
